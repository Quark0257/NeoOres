...
public class RecursiveGenerator {
	...
	public synchronized void getRecursiveList(StructurePiece startPiece, BlockPos initialPos, Rotation rot, @Nullable BlockPos usedPort, Random rand, int limit) {
		if(startPiece == null) return;
		if(this.maxBox == null) this.maxBox = new StructureBoundingBox(initialPos.getX() - range,initialPos.getY() - range,initialPos.getZ() - range,initialPos.getX() + range,initialPos.getY() + range,initialPos.getZ() + range);
		if(initialPos.getY() < 0 || 256 <= initialPos.getY() + startPiece.size.getY()) return;
		if(startPiece.hasChest) this.chestCount++;
		StructureBoundingBox sbb = new StructureBoundingBox(initialPos, startPiece.size, rot);
		if(sbb.maxY > 255 || sbb.minY < 0) return;
		...
		for(Entry<Type, Map<EnumFacing,List<BlockPos>>> facePoss : startPiece.getPorts().entrySet()) {
			for(Entry<EnumFacing,List<BlockPos>> poss : facePoss.getValue().entrySet()) {
				for(BlockPos pos : poss.getValue()) {
					if(usedPort != null) {
						if(usedPort.equals(pos)) continue;
					}
					if(startPiece.getNextConnectable(facePoss.getKey()) == null) return;
					...
					for(SPOsAndWeight spoaw : spoaws0) {
						...
						for(StructurePieceAndOption spao : spoaw.list) {
							...
							for(StructureBoundingBox bb : bblist) {
								...
								if(!bb.doesExclude(sbb_part)) continue loop0;
								if(!this.maxBox.doesIncludeAll(sbb_part)) continue loop0;
							}
							spaos.add(spao);
						}
						if(!spaos.isEmpty()) spoaws.add(new SPOsAndWeight(spaos, spoaw.weight));
					}
					if(spoaws.isEmpty()) continue;
					int weights = 0;
