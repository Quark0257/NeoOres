...
public class ItemStackWithSizeForRecipe 
{
	...
	public boolean isEmpty()
	{
		...
		if(this.isItemStack()) return ((ItemStack)this.object).isEmpty();
	...
	{
		List<ItemStack> list = new ArrayList<ItemStack>();
		if(this.isItemStack()) list.add(getStack());
		else if(this.isOreDic()) list.addAll(getListFromOreDic());
		return list;
	}
	...
	public boolean compareOreDicWith(ItemStack stack)
	{
		if(this.isOreDic())
		{
			...
			{
				if(compareWith(stack,stack1)) return true;
			}
		}
		return false;
	}
	...
	{
		if(this.isOreDic()) return this.compareOreDicWith(stack);
		else if(this.isItemStack()) return this.compareStackWith(stack);
		return false;
	}
	/**
    ...
    {
    	return (!stack1.hasTagCompound() && stack2.hasTagCompound()) ? false : ((!stack1.hasTagCompound() && !stack2.hasTagCompound() || stack1.hasTagCompound() && !stack2.hasTagCompound()) ? true : stack1.getTagCompound().equals(stack2.getTagCompound()));
    }
