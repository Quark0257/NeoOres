...
public class StructureBoundingBox {
	...
	public boolean doesInclude(StructureBoundingBox compared) {
		...
		for(BlockPos pos : compared.getVertexes()) {
			if(this.minX <= pos.getX() && pos.getX() <= this.maxX 
					&& this.minY <= pos.getY() && pos.getY() <= this.maxY
					&& this.minZ <= pos.getZ() && pos.getZ() <= this.maxZ) {
				flag0 = true;
				break;
			}
		}
		for(BlockPos pos : this.getVertexes()) {
			if(compared.minX <= pos.getX() && pos.getX() <= compared.maxX 
					&& compared.minY <= pos.getY() && pos.getY() <= compared.maxY
					&& compared.minZ <= pos.getZ() && pos.getZ() <= compared.maxZ) {
	...
	public boolean doesIncludeAll(StructureBoundingBox included) {
		return included.maxX <= this.maxX && this.minX <= included.minX
				&& included.maxY <= this.maxY && this.minY <= included.minY
				&& included.maxZ <= this.maxZ && this.minZ <= included.minZ;
	}
	
	public boolean doesExclude(StructureBoundingBox compared) {
		return compared.maxX < this.minX || this.maxX < compared.minX
				|| compared.maxY < this.minY || this.maxY < compared.minY
				|| compared.maxZ < this.minZ || this.maxZ < compared.minZ;
	}
