...
public class BiomeDecoratorSky extends BiomeDecorator
{

	private final boolean isWaterAir;

	public BiomeDecoratorSky(boolean isWaterAir)
	{
	...
	public List<BlockPos> heightList(int x, int z, World worldIn, Biome biomeIn)
	{
		...
		for (int y = 0; y < 255; y++)
		{
			...
			if (worldIn.getBlockState(blockpos) == biomeIn.topBlock
					&& (this.isWaterAir ? (worldIn.getBlockState(blockpos.add(0, 1, 0)).getBlock() == Blocks.FLOWING_WATER || worldIn.getBlockState(blockpos.add(0, 1, 0)).getBlock() == Blocks.WATER)
