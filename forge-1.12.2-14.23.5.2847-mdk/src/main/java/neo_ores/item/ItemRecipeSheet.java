...
public class ItemRecipeSheet extends INeoOresItem.Impl implements ISpellRecipeWritable
{
	...
	{
		if (!playerIn.world.isRemote || !this.hasRecipe(playerIn.getHeldItem(handIn)))
		...
		for (RecipeOreStack recipe : SpellUtils.getClumpedRecipeFromList(this.readRecipeSpells(playerIn.getHeldItem(handIn))))
		{
			...
			{
				ITextComponent itextcomponent0 = new TextComponentString(
						recipe.getStack().getDisplayName() + " (" + recipe.getStack().getItem().getRegistryName().getResourceDomain() + ")" + " : x" + recipe.getSize());
