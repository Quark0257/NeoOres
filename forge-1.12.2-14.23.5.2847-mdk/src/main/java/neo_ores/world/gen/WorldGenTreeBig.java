...
import neo_ores.block.BlockDimensionPillar.EnumAxis;
...
public class WorldGenTreeBig extends WorldGenAbstractTree {
	private Random rand;
    private World world;
    ...
    List<WorldGenTreeBig.FoliageCoordinates> foliageCoords;
    private final IBlockState trunk;
    private final IBlockState leaf;

    public WorldGenTreeBig(boolean notify, IBlockState trunk, IBlockState leaf)
    {
        super(notify);
        this.trunk = trunk;
        this.leaf = leaf;
    ...
    public static WorldGenTreeBig make(boolean notify, Block trunk, Block leaf, DimensionName dim) {
    	...
    	return new WorldGenTreeBig(notify,trunkState,leafState);
    }
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<WorldGenTreeBig.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new WorldGenTreeBig.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new WorldGenTreeBig.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void limb(BlockPos p_175937_1_, BlockPos p_175937_2_)
    {
        ...
        for (int j = 0; j <= i; ++j)
        {
            ...
            this.setBlockAndNotifyAdequately(this.world, blockpos1, this.trunk.withProperty(BlockDimensionLog.LOG_AXIS, blocklog$enumaxis));
        }
    }
    ...
    void generateLeaves()
    {
        for (WorldGenTreeBig.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
        {
    ...
    void generateLeafNodeBases()
    {
        for (WorldGenTreeBig.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
        {
            ...
            {
                this.limb(blockpos, worldgenbigtree$foliagecoordinates);
            }
        }
    }
