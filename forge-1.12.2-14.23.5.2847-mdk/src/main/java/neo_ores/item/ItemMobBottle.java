...
public class ItemMobBottle extends INeoOresItem.Impl implements IPostscriptDataIntoSpell
{
	...
	/**
	 * required runner life equals summoning entity life.
	 * it means when spell runs, player is damaged as much as summoning entity life.
	 */
	@Override
	public NBTTagCompound postscript(ItemStack stack, World world,NBTTagCompound nbt) 
	{
		if(!stack.hasTagCompound()) return nbt;
	...
	public NBTTagList addFormattedDesc(ItemStack stack,World world,NBTTagList nbt)
	{
		...
		if(entity == null || !(entity instanceof EntityLivingBase)) return nbt;
	...
	{
		if(world.isRemote) return new ActionResult<ItemStack>(EnumActionResult.FAIL, player.getHeldItem(hand));
		...
		if(result == null || result.typeOfHit != RayTraceResult.Type.BLOCK) return new ActionResult<ItemStack>(EnumActionResult.PASS, stack);
	...
	public boolean itemInteractionForEntity(ItemStack stack, EntityPlayer player, EntityLivingBase target, EnumHand hand)
	{
		...
		if(!player.isServerWorld()) return false;
		...
		NBTUtils.ForItemStack nbtutils = new NBTUtils.ForItemStack(stack);
		if(nbtutils.getCompound("storedEntity") != null && !nbtutils.getCompound("storedEntity").equals(new NBTTagCompound())) return false;
