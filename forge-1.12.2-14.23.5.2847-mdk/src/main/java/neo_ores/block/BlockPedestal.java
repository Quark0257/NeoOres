...
public class BlockPedestal extends NeoOresBlock implements ITileEntityProvider
{
	...
	{
		if(watered)	return AABB_BOUNDING;
		else if(state.getValue(FACING) == EnumFacing.UP) return AABB_BOUNDING3;
	...
	public TileEntity createNewTileEntity(World worldIn, int meta)
	{
		...
		if(!watered) teep.offset = (this.getStateFromMeta(meta).getValue(FACING) == EnumFacing.UP) ? -0.5000D : 0.0625D;
    ...
    {
		if(!watered) return this.getDefaultState().withProperty(FACING, (placer.rotationPitch < 0) ? EnumFacing.UP : EnumFacing.DOWN);
		return this.getDefaultState();
    }
    
    public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack)
    {
    	if(!watered) worldIn.setBlockState(pos, state.withProperty(FACING, (placer.rotationPitch < 0) ? EnumFacing.UP : EnumFacing.DOWN), 2);
    }
