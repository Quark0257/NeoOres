...
public class HookEntityMobAttack extends AbstractLocalVarChanger
{
	@Override
	public MethodVisitor getVisitor(MethodVisitor mv, int access, String name, String desc)
	{
		return new MethodVisitor(Opcodes.ASM4, mv)
		{
			...
			{
				if (count == CoreConfigManager.getInstance().getConfig().mobAttacksEntityHook[0])
				{
			...
			{
				if (methodCount == CoreConfigManager.getInstance().getConfig().mobAttacksEntityHook[1] || methodCount == CoreConfigManager.getInstance().getConfig().mobAttacksEntityHook[2])
				{
					// Removing methods
				}
				else
	...
	{
		return "attackWithMob";
	}

	public String hookDescriptor()
	{
		return NeoOresTransformer.toDesc(boolean.class, "net.minecraft.entity.Entity", "net.minecraft.entity.monster.EntityMob", float.class);
	}
