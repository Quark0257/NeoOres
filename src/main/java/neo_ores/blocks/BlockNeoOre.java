...
public class BlockNeoOre extends Block
{	
	...
	public BlockNeoOre(String registername, int harvestLevel, @Nullable DimensionName dimension,float light, boolean dropSelf, @Nullable Item dropItem,int damage, int min,int max,int dropExpMin,int dropExpMax)
	{
		super(Material.ROCK);
    ...
    {
		if((new ItemStack(this)).getItem() == Item.getItemFromBlock(NeoOres.custom_lit_redstone_ore)) 
		{
			return new ItemStack(NeoOres.custom_redstone_ore,1,this.getMetaFromState(worldIn.getBlockState(pos)));
		}
    ...
    {
		if(state.getBlock() == NeoOres.custom_lit_redstone_ore) return new ItemStack(NeoOres.custom_redstone_ore,1,this.getMetaFromState(state));
    ...
    private void activate(World worldIn, BlockPos pos)
    {
        ...
        if(this == NeoOres.custom_redstone_ore) worldIn.setBlockState(pos,NeoOres.custom_lit_redstone_ore.getDefaultState().withProperty(DIM, worldIn.getBlockState(pos).getValue(DIM)));
    }

    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
    	if(state.getBlock() == NeoOres.custom_lit_redstone_ore) worldIn.setBlockState(pos,NeoOres.custom_redstone_ore.getDefaultState().withProperty(DIM, worldIn.getBlockState(pos).getValue(DIM)));
    }
    ...
    {
    	return this == NeoOres.custom_lit_redstone_ore;
    }
    
    private boolean isRedstoneOre()
    {
    	return this == NeoOres.custom_redstone_ore;
    }
	...
	{
		return dimension != null ? "tile." + this.getRegistryName().getResourcePath() : "tile." + DimensionName.getFromMeta(stack.getItemDamage()).getName() + "_" + this.getRegistryName().getResourcePath();
	}
