...
public class BlockDimensionSaplingWater extends BlockDimensionSapling {
	...
	public boolean canBlockStay(World worldIn, BlockPos pos, IBlockState state)
    {
        if (state.getBlock() == this) //Forge: This function is called during world gen and placement, before this block is set, so if we are not 'here' then assume it's the pre-check.
        {
            ...
            return soil.getBlock() == this.sustainState && (worldIn.getBlockState(pos.add(0, 1, 0)).getBlock() == Blocks.FLOWING_WATER || worldIn.getBlockState(pos.add(0, 1, 0)).getBlock() == Blocks.WATER);
        }
        return this.canSustainBush(worldIn.getBlockState(pos.down())) && (worldIn.getBlockState(pos.add(0, 1, 0)).getBlock() == Blocks.FLOWING_WATER || worldIn.getBlockState(pos.add(0, 1, 0)).getBlock() == Blocks.WATER);
    }
