...
public class WorldGenSimpleTree extends WorldGenAbstractTree {
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		...
		for(int y0 = 0;y0 < base;y0++) {			
			if(!this.isReplaceable(worldIn, position.add(0, y0, 0))) return false;
			logs.add(position.add(0, y0, 0));
		}
		List<BlockPos> leaves1 = new ArrayList<BlockPos>();
		for(int x = 0;x < 3;x++) {
			for(int z = 0;z < 3;z++) {
				if(!this.isReplaceable(worldIn, position.add(x - 1, base + 2, z - 1))) return false;
				leaves1.add(position.add(x - 1, base + 1, z - 1));
			}
		}
		for(int x = 0;x < 5;x++) {
			for(int z = 0;z < 5;z++) {
				if(!this.isReplaceable(worldIn, position.add(x - 2, base + 1, z - 2))) return false;
				leaves1.add(position.add(x - 2, base, z - 2));
			}
		}
		for(int x = 0;x < 7;x++) {
			for(int z = 0;z < 7;z++) {
				if(18 <= (x - 3) * (x - 3) + (z - 3) * (z - 3)) continue;
				if(!this.isReplaceable(worldIn, position.add(x - 3, base + 1, z - 3))) return false;
