...
public class BlockDimensionLeaves extends Block implements INeoOresBlock,IShearable {
	...
	public BlockDimensionLeaves(Material materialIn, String registername) {
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(BlockDimension.DIM, DimensionName.EARTH).withProperty(CHECK_DECAY, Boolean.valueOf(true)).withProperty(DECAYABLE, Boolean.valueOf(true)));
	}
	
	public int getFireSpreadSpeed(IBlockAccess world, BlockPos pos, EnumFacing face)
    {
		if(world.getBlockState(pos).getValue(BlockDimension.DIM) == DimensionName.FIRE || world.getBlockState(pos).getValue(BlockDimension.DIM) == DimensionName.WATER) return 0; 
		return 30;
    }
    ...
    {
    	int i = 0;
    	if(NeoOresBlocks.dim_leaves == state.getBlock()) i++;
    	if(NeoOresBlocks.corroding_dim_leaves == state.getBlock()) i = i + 2;
    	i = state.getValue(BlockDimension.DIM).getColorIndex() * 3 + i;
    	for(int j = 0;j < NeoOresBlocks.color_saplings.size();j++) {
    		if(i == j) return Item.getItemFromBlock(NeoOresBlocks.color_saplings.get(j));
    	}
        return Items.APPLE;
    }
    ...
    {
        return this.getDefaultState().withProperty(BlockDimension.DIM, DimensionName.getFromMeta(meta % 4)).withProperty(DECAYABLE, Boolean.valueOf((meta & 4) == 0)).withProperty(CHECK_DECAY, Boolean.valueOf((meta & 8) > 0));
    }
