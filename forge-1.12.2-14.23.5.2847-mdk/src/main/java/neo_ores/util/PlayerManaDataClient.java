...
public class PlayerManaDataClient
{
	...
	{
		if (value + (float) this.getMana() > Long.MAX_VALUE)
			return this.setMana(Long.MAX_VALUE);
		else if (value + this.getMana() >= 0)
			return this.setMana(value + this.getMana());
		else
			return this.setMana(0);
	}

	public NBTTagCompound addLevel(long value)
	{
		if (value + this.getLevel() > Long.MAX_VALUE)
			return this.setLevel(Long.MAX_VALUE);
		else if (value + this.getLevel() >= 0)
			return this.setLevel(value + this.getLevel());
		else
	...
	{
		if (value + this.getMXP() > Long.MAX_VALUE)
			return this.setMXP(Long.MAX_VALUE);
		else if (value + this.getMXP() >= 0)
			return this.setMXP(value + this.getMXP());
		else
			return this.setMXP(0);
	}

	public NBTTagCompound addMaxMana(long value)
	{
		if (value + this.getMaxMana() > Long.MAX_VALUE)
			return this.setMaxMana(Long.MAX_VALUE);
		else if (value + this.getMaxMana() >= 0)
			return this.setMaxMana(value + this.getMaxMana());
		else
			return this.setMaxMana(0);
	}

	public NBTTagCompound addMagicPoint(long value)
	{
		if (value + this.getMagicPoint() > Long.MAX_VALUE)
			return this.setMagicPoint(Long.MAX_VALUE);
		else if (value + this.getMagicPoint() >= 0)
			return this.setMagicPoint(value + this.getMagicPoint());
		else
			return this.setMagicPoint(0);
	}

	public void sendToServer(NBTTagCompound magicnbt, NBTTagCompound studynbt)
	{
		if (magicnbt != null)
			this.neo_ores.setTag(SpellUtils.NBTTagUtils.MAGIC, magicnbt.copy());
		if (studynbt != null)
			this.neo_ores.setTag(SpellUtils.NBTTagUtils.STUDY, studynbt.copy());
