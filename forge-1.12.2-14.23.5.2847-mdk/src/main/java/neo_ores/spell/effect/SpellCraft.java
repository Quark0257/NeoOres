			...
			if (state.getBlock() == Blocks.CRAFTING_TABLE)
			{
				...
				if (StructureUtils.isMatch(world, str, ICompareBlockState.DEFAULT))
				{
					...
					if (recipe != null)
					{
						...
						EntityItem entityItem = new EntityItem(world, pos.getX() + 0.5D, pos.getY() + 2.5D, pos.getZ() + 0.5D, recipeResult.copy());
				...
				if (StructureUtils.isMatch(world, str, ICompareBlockState.DEFAULT))
				{
					...
					for (int z = -1; z <= 1; z++)
					{
						for (int x = -1; x <= 1; x++)
						{
							...
							if (te instanceof TileEntityPedestal)
							{
								stacks.add(((TileEntityPedestal) te).getStackInSlot(0));
							}
						}
					}
					Tuple<ItemStack, Long> recipeResult = ManaCraftingRecipes.getResult(stacks);
					if (!recipeResult.getFirst().isEmpty() && recipeResult.getSecond() < Math.pow(8, this.tier))
					{
						EntityItem entityItem = new EntityItem(world, pos.getX() + 0.5D, pos.getY() + 2.5D, pos.getZ() + 0.5D, recipeResult.getFirst().copy());
						entityItem.motionX = 0.0;
						entityItem.motionY = 0.0;
						entityItem.motionZ = 0.0;
						world.spawnEntity(entityItem);
